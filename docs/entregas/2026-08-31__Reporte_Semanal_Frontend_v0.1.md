...
| **¿Qué está en curso?** | Formalización de los documentos de entrega (minuta, acta de roles, registro de decisiones, matriz de dependencias, inventario técnico) en `docs/entregas/`. Home navegable con datos simulados implementada en fases previas. |
...
| **¿Qué entregable/documento actualizamos?** | `docs/entregas/` (minuta, acta de roles, registro de decisiones, matriz de dependencias, reporte, inventario técnico); `docs/frontend/` (propuesta, acta, agenda, dependencias). |
...
| Pregunta | Respuesta |
|----|----|
| **¿Qué terminamos esta semana?** | Auditoría de infraestructura completa (`docs/sdd/06_auditorias/auditoria-infraestructura.md`): 10 brechas identificadas (INF-A1…INF-J1), 1 severa (typo de dominio en `grupo_5`, corregido en el Drive). Rama `feature/implementacion-infra`: propuesta tecnológica §6 alineada con la especificación (versiones mínimas, ancho de banda, subdominios `test`/`campus`, TLS 1.2/1.3 + HSTS), nueva §9 con política de imágenes, README con build de producción. Bloque Nginx de ejemplo completado con cabeceras de seguridad y `client_max_body_size 15m`. Recordatorio `docs/driveFrontend/05_infraestructura/bloqueo_infra.md` para Infra. PR #2 abierto hacia `develop`. |
| **¿Qué está en curso?** | Alineación de entregables de `docs/entregas/` (matriz de dependencias, registro de decisiones, reporte) y conversión a Markdown. Fase 6 (brechas de auditoría de análisis) ya implementada en fases previas. |
...
| **¿Qué entregable/documento actualizamos?** | `docs/entregas/` (matriz de dependencias, registro de decisiones, reporte semanal — en Markdown); `docs/frontend/` (auditoría de infra, propuesta tecnológica, estado del proyecto, vitácora); `docs/driveFrontend/05_infraestructura/` (bloqueo_infra.md, grupo_5 corregido). |
