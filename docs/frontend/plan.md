...
- `docs/driveFrontend/Minuta Analisis Funcional IFTS 12 - 27-08-2026.pdf` — relevamiento
...
- `docs/driveFrontend/Tarea Inicial - Frontend.docx` — objetivo, entregables y coordinación
  con otros equipos; reunión de revisión el lunes 31/08/2026.
- `docs/driveFrontend/Integrantes y Roles - Frontend.docx` — integrantes y roles del área
  (no se modifica; la asignación actualizada vive en `docs/rol_equipo.md`).
- `docs/driveFrontend/Plan_B_Infraestructura_VPS_IFTS12_2026.docx` — el Moodle no se migra;
  la nueva web se aloja en infraestructura independiente (VPS) y Frontend entrega un build
  estático servido desde Nginx.
- `docs/driveFrontend/PAUTAS_PRACTICA_PROFESIONAL_INTEGRADORA_IFTS12_2026.docx` — entregables
  obligatorios del área (sección 10.3) y plantillas de documentación (sección 16).

Nota: `docs/driveFrontend/` contiene los fuentes del Drive, no se versiona.
...
| Roles | Asignación registrada en `docs/rol_equipo.md`. |
| Docx de roles | No se modifica; queda en `docs/driveFrontend/` (no versionado). |

## 4. Fases con responsables

Roles según `docs/rol_equipo.md`. Enlace transversal: Emilio Aquino.
...
El detalle por área con responsables y estados está en `docs/frontend/dependencias-equipos.md`.
...
- `docs/driveFrontend/` no se versiona.

## 10. Antecedentes y documentos relacionados

- `docs/driveFrontend/*.docx` y `*.pdf` — fuentes de verdad (no versionadas).
- `docs/frontend/propuesta-tecnologica.md` — stack frontend + infraestructura del VPS.
- `docs/frontend/dependencias-equipos.md` — bloqueos y decisiones por área.
- `docs/rol_equipo.md` — asignación de roles del equipo Frontend.
